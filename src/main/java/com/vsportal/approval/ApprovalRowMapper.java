...
public class ApprovalRowMapper<T> implements RowMapper<Approval> {
	public Approval mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		Client cl = new Client(rs.getInt("Approval.client_id"), rs.getString("clientid.name"));
		approval.setClient(cl);
		
		User cb = new User(rs.getInt("Approval.created_by"), rs.getString("createdby.full_name"));
		approval.setCreatedBy(cb);
		
		User db = new User(rs.getInt("Approval.decision_by"), rs.getString("decisionby.full_name"));
		approval.setDecisionBy(db);
		
		User ub = new User(rs.getInt("Approval.updated_by"), rs.getString("updatedby.full_name"));
		approval.setUpdatedBy(ub);
		
		Request req = new Request(rs.getInt("Approval.request_id"), rs.getString("requestid.req_nbr"));
