	...
	function attachRules(formDefinition) {
		...
		for ( var rule in fieldRules) {
			if (fieldRules.hasOwnProperty(rule)) {
				var fieldEl = $('#' + rule.fieldId);

				// Check if field exists
				if (fieldEl.length) {
					if (fieldEl.is('input')) {
						if (fieldEl.attr('type') == 'checkbox'
								|| fieldEl.attr('type') == 'file'
								|| fieldEl.attr('type') == 'hidden') {
		...
		for ( var rule in fieldRules) {
			if (fieldRules.hasOwnProperty(rule)) {
				...
				if (fieldEl.length) {
					...
					//If element not found, it is a radio button
					fieldEl = $('input[name="' + rule.fieldId + '"]:radio').first();
				}
				...
				if(fieldEl.length) {
					...
					if($.inArray(sessionUser.role, rule.role.write.split(',')) == -1) {
						...
						//User does have listed role, don't restrict write access
						fieldEl.closest('fieldset').removeClass('role_no_write');
					...
					if(eval(rule.readonly)) {
						...
						if(!fieldEl.closest('fieldset').hasClass('role_hidden') && !fieldEl.closest('fieldset').hasClass('role_no_write')) {
							fieldEl.closest('fieldset').prop('disabled', false);
						}
					}
					...
					if(eval(rule.mandatory)) {
						if(fieldEl.closest('fieldset').prop('disabled') == false && !fieldEl.closest('fieldset').hasClass('role_hidden')) {
							fieldEl.closest('fieldset').prop('mandatory', true);
							$('<i class="fa fa-asterisk icon-mandatory"></i>').insertBefore(fieldEl.closest('fieldset').first());
						} else {
							fieldEl.closest('fieldset').prop('mandatory', false);
