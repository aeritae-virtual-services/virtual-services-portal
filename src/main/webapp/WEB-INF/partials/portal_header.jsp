	...
	<div class="container-fluid">
		...
		<div class="collapse navbar-collapse">
			<ul class="nav navbar-nav navbar-right">
				<c:if test='${valid_session eq "true"}'>
					<li><a href="" data-toggle="tooltip" title="My Account"
						data-placement="bottom"> <i class="white-icon fa fa-user"></i>
					</a></li>
				</c:if>
			</ul>
				...
				<c:if test='${valid_session eq "true"}'>
					<div class="form-group">
						<div class="left-inner-addon">
							<i class="fa fa-search"></i> <input id="navbar-global-search"
			...
			<ul class="nav navbar-nav navbar-right">
				<c:if test='${valid_session eq "true"}'>
					<li><a href="" data-toggle="tooltip" title="View Pokes"
						data-placement="bottom" onclick="return togglePokes();"> <i
							class="white-icon fa fa-hand-o-right"></i> <c:if
								test='${numPokes ne 0}'>
								<span id="pokeCount" class="badge orange-badge"> ${numPokes} </span>
							</c:if>
					</a>
						<div id="poke-view" class="poke-view" style="display: none;">
							<h4>
								<i class="fa fa-hand-o-right" style="margin-right: 10px;"></i>My
								Pokes
							</h4>
									...
									<table>
										<tbody>
											<tr>
												<td>
													<h5>
														<a href="/update_task/id=${pokedTask.id}">${pokedTask.number}</a>
														- ${pokedTask.client.displayValue}
													</h5>
													<h6>${pokedTask.instructions}</h6>
												</td>
												<td
													style="padding: 10px; text-align: center; white-space: nowrap;">
													<a onclick="acceptPoke(this, '${pokedTask.id}')"
													class="icon-save"> <i class="fa fa-check"></i>
												</a> <a onclick="declinePoke(this, '${pokedTask.id}')"
													class="icon-cancel"> <i class="fa fa-times"></i>
												</a>
												</td>
											</tr>
										</tbody>
					...
					<li><a href="" data-toggle="tooltip"
						title="View Notifications" data-placement="bottom"
						onclick="return toggleNotifications();"> <i
							class="white-icon fa fa-bell"></i> <span
							class="badge orange-badge">1</span>
					</a>
						<div id="notification-view" class="notification-view"
							style="display: none;">
							<h4>
								<i class="fa fa-bell" style="margin-right: 10px;"></i>My
								Notifications
							</h4>
								...
								<table>
									<tbody>
										<tr>
											...
											<td
												style="padding: 10px; text-align: center; white-space: nowrap;">
												<a class="icon-save"> <i class="fa fa-check"></i>
											</a>
											</td>
										</tr>
									</tbody>
	...
	function acceptPoke(el, taskId) {
		$.ajax({
			...
			success : function() {
				...
				var pokeCount = $('#pokeCount').html();
				pokeCount = parseInt(pokeCount) - 1;
				$('#pokeCount').html(pokeCount);
			},
			error : function() {
